...
int main(int argc, char *argv[]){
	...
	NetDeviceContainer staDevices3;
 	...
 	mac3.SetType ("ns3::ApWifiMac",
  	...
  	apDevices3 = wifi3.Install (phy, mac3, apNode.Get(2));
        ...
        WifiHelper wifi4;
        wifi4.SetStandard (WIFI_PHY_STANDARD_80211n_2_4GHZ);
        wifi4.SetRemoteStationManager ("ns3::IdealWifiManager", "RtsCtsThreshold", UintegerValue (999999));

        WifiMacHelper mac4;

        NodeContainer stationNode4;
        stationNode4.Create(nstas);

        mac4.SetType ("ns3::StaWifiMac","Ssid", SsidValue (ssid),"ActiveProbing", BooleanValue (true));
        phy.Set("TxPowerStart", DoubleValue(15.0));
        phy.Set("TxPowerEnd", DoubleValue(15.0));

        NetDeviceContainer staDevices4;
        staDevices4 = wifi1.Install (phy, mac4, stationNode4);
        ...
        apDevices4 = wifi4.Install (phy, mac4, apNode.Get(3));

        NodeContainer stationNode5;
        stationNode5.Create(nstas);

        WifiHelper wifi5;
        wifi5.SetStandard (WIFI_PHY_STANDARD_80211n_2_4GHZ);
        wifi5.SetRemoteStationManager ("ns3::IdealWifiManager", "RtsCtsThreshold", UintegerValue (999999));
        WifiMacHelper mac5;

        mac5.SetType ("ns3::StaWifiMac","Ssid", SsidValue (ssid),"ActiveProbing", BooleanValue (true));
        phy.Set("TxPowerStart", DoubleValue(15.0));
        phy.Set("TxPowerEnd", DoubleValue(15.0));

        NetDeviceContainer staDevices5;
        staDevices5 = wifi1.Install (phy, mac5, stationNode5);
	...
	internetstack.Install(stationNode3);
        internetstack.Install(stationNode4);
	...
	apInterface3 = ip.Assign(apDevices3);
        ...
        apInterface4 = ip.Assign(apDevices4);
	...
	staInterface3 = ip.Assign(staDevices3);
        ...
        staInterface4 = ip.Assign(staDevices4);
	...
	anim.UpdateNodeDescription(apNode.Get(0),"Ap-1");
        anim.SetConstantPosition(apNode.Get(0),300.0,400.0);

	anim.UpdateNodeDescription(apNode.Get(1),"Ap-2");
        anim.SetConstantPosition(apNode.Get(1),400.0,400.0);

	anim.UpdateNodeDescription(apNode.Get(2),"Ap-3");
        anim.SetConstantPosition(apNode.Get(2),500.0,400.0);

        anim.UpdateNodeDescription(apNode.Get(3),"Ap-4");
        anim.SetConstantPosition(apNode.Get(3),400.0,200.0);

	anim.UpdateNodeDescription(switchNode.Get(0),"Switch Node");
        anim.SetConstantPosition(switchNode.Get(0),150.0,300.0);

	anim.UpdateNodeDescription(serverNode.Get(0),"Server Node");
        anim.SetConstantPosition(serverNode.Get(0),50.0,300.0);

        anim.UpdateNodeDescription(stationNode2.Get(0),"Station-2-1 node");
        anim.SetConstantPosition(stationNode2.Get(0),280.0,470.0);
        anim.UpdateNodeDescription(stationNode2.Get(1),"Station-2-2 node");
        anim.SetConstantPosition(stationNode2.Get(1),320.0,470.0);

	anim.UpdateNodeDescription(stationNode3.Get(0),"Station-3-1 node");
        anim.SetConstantPosition(stationNode3.Get(0),380.0,470.0);
        anim.UpdateNodeDescription(stationNode3.Get(0),"Station-3-2 node");
        anim.SetConstantPosition(stationNode3.Get(1),420.0,470.0);

        anim.UpdateNodeDescription(stationNode4.Get(0),"Station-4-1 node");
        anim.SetConstantPosition(stationNode4.Get(0),480.0,470.0);
        anim.UpdateNodeDescription(stationNode4.Get(1),"Station-4-1 node");
        anim.SetConstantPosition(stationNode4.Get(1),520.0,470.0);

        anim.UpdateNodeDescription(stationNode5.Get(0),"Station-5-1 node");
        anim.SetConstantPosition(stationNode5.Get(0),380.0,250.0);
        anim.UpdateNodeDescription(stationNode5.Get(1),"Station-5-1 node");
        anim.SetConstantPosition(stationNode5.Get(1),420.0,250.0);


        /*anim.UpdateNodeDescription(stationNode1.Get(0),"Station-1-1 node");
        anim.SetConstantPosition(stationNode1.Get(0),280.0,310.0);
        anim.UpdateNodeDescription(stationNode1.Get(1),"Station-1-2 node");
        anim.SetConstantPosition(stationNode1.Get(1),320.0,310.0);*/


        //anim.SetConstantPosition(stationNode1.Get(1),200,500.0);
